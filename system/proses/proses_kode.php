...
if(isset($_POST['tambahkode']))
	...
	//for ($s=0; $s<$input; $s++) 
		//{
			$sql= "INSERT INTO `$tabelkode[0]`(`$fieldkode[0]`) VALUES ('$isi[0]')";
 			$query=mysqli_query($koneksi,$sql);
		//}

 			include "../logika/tambah_kode.php";
			}
// Rubah Data..
...
if(isset($_POST['rubah']))
	...
	for ($s=0; $s<$input; $s++) 
		{
 		$sql="UPDATE `$tabelkode[0]` SET `$fieldkode[$s]`='$isi[$s]'
 		WHERE `$tabelkode[0]`.$kode[0]='$Kode'";
 		$query=mysqli_query($koneksi,$sql);
		}
	
 		include "../logika/kelola_kode.php";
    }
...
// Hapus Data..
 ...
 for ($sn=0; $sn<$n; $sn++) 
		{
 $sql="DELETE FROM `$tabeltoko[0]` WHERE  `$tabeltoko[0]`.`$kode[0]`='$Kode[$sn]'";
 $query=mysqli_query($koneksi,$sql);
 		}
...
// Format Data..
 ...
 $sql="TRUNCATE `$database`.`$tabeltoko[0]`";
 $query=mysqli_query($koneksi,$sql);
